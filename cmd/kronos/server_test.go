...
func TestServerMetricsEndpoint(t *testing.T) {
	...
	for _, want := range []string{
		...
		`kronos_targets_by_driver{driver="redis"} 2`,
		`kronos_storages_total 2`,
		`kronos_storages_by_kind{kind="local"} 1`,
		`kronos_storages_by_kind{kind="s3"} 1`,
		`kronos_schedules_total 2`,
		`kronos_schedules_paused 1`,
		`kronos_schedules_by_type{type="full"} 1`,
		`kronos_schedules_by_type{type="incr"} 1`,
...
func TestServerJobsEndpoint(t *testing.T) {
	...
	if err := store.Save(core.Job{ID: "job-4", Operation: core.JobOperationRestore, AgentID: "agent-1", TargetID: "restore-target", StorageID: "storage-1", RestoreBackupID: "backup-1", RestoreManifestID: "manifest-1", RestoreDryRun: true, Status: core.JobStatusSucceeded, QueuedAt: now.Add(-3 * time.Hour)}); err != nil {
		t.Fatalf("Save(job-4) error = %v", err)
	}
	...
	resp, err = server.Client().Get(server.URL + "/api/v1/jobs?operation=restore&restore_backup_id=backup-1")
	if err != nil {
		t.Fatalf("GET backup restore jobs error = %v", err)
	}
	body.Reset()
	if _, err := body.ReadFrom(resp.Body); err != nil {
		t.Fatalf("ReadFrom(backup restore jobs) error = %v", err)
	}
	resp.Body.Close()
	if !strings.Contains(body.String(), `"id":"job-4"`) || strings.Contains(body.String(), `"id":"job-1"`) {
		t.Fatalf("backup restore jobs body = %q", body.String())
	}
...
func TestServerRejectsBadRequestsAndMethods(t *testing.T) {
	...
	cases := []struct {
		...
		{method: http.MethodGet, path: "/api/v1/jobs?since=2026-04-26T00:00:00Z&until=2026-04-25T00:00:00Z", status: http.StatusBadRequest},
		{method: http.MethodGet, path: "/api/v1/backups?since=2026-04-26T00:00:00Z&until=2026-04-25T00:00:00Z", status: http.StatusBadRequest},
		{method: http.MethodGet, path: "/api/v1/audit?since=2026-04-26T00:00:00Z&until=2026-04-25T00:00:00Z", status: http.StatusBadRequest},
