...
func TestRunHelp(t *testing.T) {
	...
	text := out.String()
	for _, want := range []string{"Kronos", "Global Flags", "--server", "--token", "--output", "--request-id", "json, pretty, yaml, or table", "--no-color", "server", "agent", "audit", "health", "ready", "jobs", "local", "metrics", "notification", "schedule", "storage", "target", "config", "version"} {
