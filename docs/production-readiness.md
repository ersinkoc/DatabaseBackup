...
cancel/retry, restore preview/dry-run queueing, and backup protection actions,
and multi-instance control-plane operation is still roadmap work.
MySQL/MariaDB
...
3. Extend E2E coverage into more retention policy edge cases and release
   verification drills.
4. Expand the WebUI from dashboard shell into live resource CRUD,
   non-dry-run restore workflow controls, and richer backup drill actions.
