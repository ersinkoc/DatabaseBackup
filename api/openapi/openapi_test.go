...
func TestOpenAPIDocumentsBackupProtectionNotFoundResponses(t *testing.T) {
	...
	text := string(data)
	for _, want := range []string{"/api/v1/backups/{id}/protect", "Enable manual protection", "/api/v1/backups/{id}/verify", "Enqueue backup verification", "FailureEvidence", "failure_evidence", "RestoreReport", "restore_report", "VerificationReport", "verify_report", "restore_backup_id", "verify_backup_id", "/api/v1/backups/{id}/unprotect", "Disable manual protection", "Backup not found"} {
		if !strings.Contains(text, want) {
			t.Fatalf("openapi.yaml missing %q", want)
		}
	}
